...
<header class="mb-8">
    ...
    <h2 class="font-headline text-3xl font-black text-white tracking-tight uppercase">Editar Rutina: <span class="text-brand-accent font-headline">{{ $routine->name }}</span></h2>
    <p class="text-sm text-slate-400 font-medium">Modifica los detalles, objetivos o la asignación de atleta para este plan de entrenamiento.</p>
</header>


<section class="max-w-3xl neon-card p-8 rounded-2xl">
    <form action="{{ route('routines.update', $routine->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="client_id" id="client_id" required
                    ...
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" {{ old('client_id', $routine->client_id) == $client->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <select name="difficulty" id="difficulty" required
                class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white focus:outline-none transition-all duration-300 text-sm">
                <option value="beginner" {{ old('difficulty', $routine->difficulty) === 'beginner' ? 'selected' : '' }}>Principiante (Básico)</option>
                <option value="intermediate" {{ old('difficulty', $routine->difficulty) === 'intermediate' ? 'selected' : '' }}>Intermedio (Medio)</option>
                <option value="advanced" {{ old('difficulty', $routine->difficulty) === 'advanced' ? 'selected' : '' }}>Avanzado (Alto Rendimiento)</option>
            </select>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="6" required
                class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white focus:outline-none transition-all duration-300 text-sm font-sans leading-relaxed">{{ old('description', $routine->description) }}</textarea>
        ...
        <div class="border-t border-slate-800/80 pt-6 flex justify-end gap-3">
            ...
            <button type="submit" class="bg-gradient-to-r from-brand-accent to-cyan-600 text-brand-darkest px-8 py-3 rounded-xl hover:shadow-[0_0_20px_rgba(0,240,255,0.4)] hover:scale-105 transition-all duration-300 font-bold font-display text-sm flex items-center gap-2">
                <span class="material-symbols-outlined text-[20px] font-black">save</span> Actualizar Rutina
            </button>
        </div>
    </form>
</section>
