...
<header class="mb-8">
    ...
    <h2 class="font-headline text-3xl font-black text-white tracking-tight uppercase">Registrar Nueva <span class="text-brand-accent font-headline">Sede</span></h2>
    <p class="text-sm text-slate-400 font-medium">Introduce la información de la nueva sucursal del gimnasio.</p>
</header>
    ...
    <form action="{{ route('branches.store') }}" method="POST">
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Ej. NovaGym Sede Norte"
            ...
            <div>
                ...
                <input type="text" name="address" id="address" value="{{ old('address') }}" placeholder="Ej. Av. Principal #123, Col. Centro"
                    class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm">
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone') }}" placeholder="Ej. +52 555 123 4567"
                        class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm">
                </div>
                <div>
                    ...
                    <input type="text" name="schedule" id="schedule" value="{{ old('schedule') }}" placeholder="Ej. Lun-Vie 6:00-22:00"
                        class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm">
                </div>
                <div>
        ...
        <div class="border-t border-slate-800/80 pt-6 mt-8 flex justify-end gap-3">
            ...
            <button type="submit" class="bg-gradient-to-r from-brand-accent to-cyan-600 text-brand-darkest px-8 py-3 rounded-xl hover:shadow-[0_0_20px_rgba(0,240,255,0.4)] hover:scale-105 transition-all duration-300 font-bold font-display text-sm flex items-center gap-2">
                <span class="material-symbols-outlined text-[20px] font-black">save</span> Guardar Sede
            </button>
        </div>
    </form>
</section>
