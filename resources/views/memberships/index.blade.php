...
<header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <p class="text-sm text-slate-400 font-medium">Administra los planes de membresía de los atletas.</p>
    </div>
    <div>
        <a href="{{ route('memberships.create') }}" class="bg-gradient-to-r from-brand-accent to-cyan-600 text-brand-darkest font-display font-bold px-5 py-2.5 rounded-xl hover:shadow-[0_0_20px_rgba(0,240,255,0.4)] hover:scale-105 transition-all duration-300 flex items-center gap-2 shadow-[0_0_15px_rgba(0,240,255,0.15)] text-sm">
            <span class="material-symbols-outlined text-[20px] font-black">add_card</span> Nueva Membresía
        </a>
    </div>
</header>
                ...
                <tr class="border-b border-slate-800/40 hover:bg-slate-800/30 transition-all duration-200">
                    ...
                    <td class="py-4 px-6 text-sm">
                        @switch($membership->plan_type)
                            @case('monthly')
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-cyan-950/40 text-brand-accent">Mensual</span>
                                @break
                            @case('quarterly')
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-blue-950/40 text-blue-400">Trimestral</span>
                                @break
                            @case('annual')
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-purple-950/40 text-purple-400">Anual</span>
                                @break
                            @default
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-slate-900/40 text-slate-400">Custom</span>
                        @endswitch
                    </td>
                    
                    <td class="py-4 px-6 text-sm font-bold text-brand-accent">
                        ${{ number_format($membership->price, 2) }}
                    </td>
                    
                    <td class="py-4 px-6 text-sm text-slate-300">
                        {{ $membership->instructor->name ?? 'Asignar' }}
                    </td>
                        ...
                        @switch($membership->status)
                            ...
                            @case('expired')
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-red-950/40 text-red-400 border border-red-900/30">
                                    <span class="w-1.5 h-1.5 rounded-full bg-red-500 shadow-[0_0_6px_#ef4444]"></span> Vencida
                                </span>
                                @break
                            @default
                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-slate-900/40 text-slate-400">
                                    <span class="w-1.5 h-1.5 rounded-full bg-slate-500"></span> Cancelada
                                </span>
                        @endswitch
                    </td>
                    ...
                    <td class="py-4 px-6 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('memberships.edit', $membership->id) }}" class="w-9 h-9 flex items-center justify-center bg-slate-900 border border-slate-800 text-slate-400 hover:text-brand-accent hover:border-brand-accent hover:shadow-[0_0_10px_rgba(0,240,255,0.25)] rounded-lg transition-all duration-200" title="Editar Membresía">
                                <span class="material-symbols-outlined text-[18px]">edit</span>
                            </a>
                            
                            <form action="{{ route('memberships.destroy', $membership->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta membresía?');" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-9 h-9 flex items-center justify-center bg-slate-900 border border-slate-800 text-slate-400 hover:text-red-400 hover:border-red-500/50 hover:shadow-[0_0_10px_rgba(239,68,68,0.25)] rounded-lg transition-all duration-200" title="Eliminar Membresía">
    ...
    $(document).ready(function() {
        ...
        $('#statusFilter').on('change', function() {
            ...
            if (val === 'all') {
                ...
                table.column(6).search(val.charAt(0).toUpperCase() + val.slice(1)).draw();
            }
        });
    });
</script>
