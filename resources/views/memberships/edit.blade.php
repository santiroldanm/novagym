...
<header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        <h2 class="font-headline text-3xl font-black text-white tracking-tight uppercase">Editar <span class="text-brand-accent cyan-glow-text font-headline">Membresía</span></h2>
        <p class="text-sm text-slate-400 font-medium">Modifica los datos del plan de {{ $membership->client->name ?? 'N/A' }}.</p>
    </div>
    <div>
    ...
    <form action="{{ route('memberships.update', $membership->id) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="plan_name" value="{{ old('plan_name', $membership->plan_name) }}" required class="w-full px-4 py-3 rounded-xl bg-brand-darkest border border-brand-border text-white focus:outline-none focus:border-brand-accent focus:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all duration-300 text-sm">
            </div>
        ...
        <div>
            ...
            <input type="number" name="price" value="{{ old('price', $membership->price) }}" required step="0.01" min="0" class="w-full px-4 py-3 rounded-xl bg-brand-darkest border border-brand-border text-white focus:outline-none focus:border-brand-accent focus:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all duration-300 text-sm">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="date" name="start_date" value="{{ old('start_date', $membership->start_date->format('Y-m-d')) }}" required class="w-full px-4 py-3 rounded-xl bg-brand-darkest border border-brand-border text-white focus:outline-none focus:border-brand-accent focus:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all duration-300 text-sm">
            </div>
            
            <div>
                <label class="block text-[11px] font-display font-black text-brand-accent uppercase tracking-widest mb-2">Fecha de Fin</label>
                <input type="date" name="end_date" value="{{ old('end_date', $membership->end_date->format('Y-m-d')) }}" required class="w-full px-4 py-3 rounded-xl bg-brand-darkest border border-brand-border text-white focus:outline-none focus:border-brand-accent focus:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all duration-300 text-sm">
            </div>
        </div>

        <div>
            <label class="block text-[11px] font-display font-black text-brand-accent uppercase tracking-widest mb-2">Instructor Asignado</label>
            <select name="instructor_id" class="w-full px-4 py-3 rounded-xl bg-brand-darkest border border-brand-border text-white focus:outline-none focus:border-brand-accent focus:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all duration-300 text-sm appearance-none">
                ...
                @foreach($instructors as $instructor)
                    <option value="{{ $instructor->id }}" {{ old('instructor_id', $membership->instructor_id) == $instructor->id ? 'selected' : '' }}>{{ $instructor->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <textarea name="notes" rows="3" placeholder="Observaciones adicionales..." class="w-full px-4 py-3 rounded-xl bg-brand-darkest border border-brand-border text-white focus:outline-none focus:border-brand-accent focus:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all duration-300 text-sm resize-none">{{ old('notes', $membership->notes) }}</textarea>
        </div>
