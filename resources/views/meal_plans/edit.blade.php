...
@section('title', 'Editar Plan de Alimentación')
...
<header class="mb-8">
    ...
    <h2 class="font-headline text-3xl font-black text-white tracking-tight uppercase">Editar Plan de <span class="text-brand-accent font-headline">Alimentación</span></h2>
    <p class="text-sm text-slate-400 font-medium">Modifica el plan nutricional <strong class="text-white">{{ $mealPlan->name }}</strong>.</p>
</header>
    ...
    <form action="{{ route('meal-plans.update', $mealPlan->id) }}" method="POST">
        ...
        <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="client_id" id="client_id" required
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('client_id', $mealPlan->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $mealPlan->name) }}" required
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div>
                    ...
                    <input type="number" name="calories" id="calories" value="{{ old('calories', $mealPlan->calories) }}"
                        class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm">
                </div>
                <div>
                    ...
                    <input type="number" name="proteins_g" id="proteins_g" value="{{ old('proteins_g', $mealPlan->proteins_g) }}"
                        class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm">
                </div>
                <div>
                    ...
                    <input type="number" name="carbs_g" id="carbs_g" value="{{ old('carbs_g', $mealPlan->carbs_g) }}"
                        class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm">
                </div>
                <div>
                    ...
                    <input type="number" name="fats_g" id="fats_g" value="{{ old('fats_g', $mealPlan->fats_g) }}"
                        class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="8" required
                    class="focus-cyan w-full bg-slate-950/60 border border-slate-800 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:outline-none transition-all duration-300 text-sm resize-none">{{ old('description', $mealPlan->description) }}</textarea>
            </div>
        </div>
        ...
        <div class="border-t border-slate-800/80 pt-6 mt-8 flex justify-end gap-3">
            ...
            <button type="submit" class="bg-gradient-to-r from-brand-accent to-cyan-600 text-brand-darkest px-8 py-3 rounded-xl hover:shadow-[0_0_20px_rgba(0,240,255,0.4)] hover:scale-105 transition-all duration-300 font-bold font-display text-sm flex items-center gap-2">
                <span class="material-symbols-outlined text-[20px] font-black">save</span> Actualizar Plan
            </button>
        </div>
    </form>
</section>
