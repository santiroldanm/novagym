...
<header class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
    <div>
        ...
        <p class="text-sm text-slate-400 font-medium">Registra, audita y administra la base de socios activos de alto rendimiento.</p>
    </div>
    <div>
        <a href="{{ route('clients.create') }}" class="bg-gradient-to-r from-brand-accent to-cyan-600 text-brand-darkest font-display font-bold px-5 py-2.5 rounded-xl hover:shadow-[0_0_20px_rgba(0,240,255,0.4)] hover:scale-105 transition-all duration-300 flex items-center gap-2 shadow-[0_0_15px_rgba(0,240,255,0.15)] text-sm">
            <span class="material-symbols-outlined text-[20px] font-black">person_add</span> Registrar Cliente
        </a>
    </div>
</header>
        ...
        <table id="clientsTable" class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-slate-800">
                    ...
                    <th class="py-4 px-6 text-[11px] font-black tracking-widest text-brand-accent uppercase font-display">Nombre Completo</th>
                    ...
                    <th class="py-4 px-6 text-[11px] font-black tracking-widest text-brand-accent uppercase font-display text-center">Rutinas Activas</th>
                ...
                <tr class="border-b border-slate-800/40 hover:bg-slate-800/30 transition-all duration-200 {{ $client->status === 'inactive' ? 'bg-red-500/5 hover:bg-red-500/10 border-red-500/10' : '' }}">
                    ...
                    <td class="py-4 px-6">
                        <div class="w-11 h-11 rounded-full overflow-hidden border-2 border-slate-800 flex items-center justify-center bg-slate-950/60 shadow-[0_0_10px_rgba(0,0,0,0.5)]">
                            @if($client->photo)
                                <img src="{{ $client->photo }}" alt="{{ $client->name }}" class="w-full h-full object-cover">
                            @else
                    ...
                    <td class="py-4 px-6 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <form action="{{ route('clients.destroy', $client->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar a {{ $client->name }}? Se eliminarán todas sus rutinas en cascada.');" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-9 h-9 flex items-center justify-center bg-slate-900 border border-slate-800 text-slate-400 hover:text-red-400 hover:border-red-500/50 hover:shadow-[0_0_10px_rgba(239,68,68,0.25)] rounded-lg transition-all duration-200" title="Eliminar Cliente">
...
<script>
    $(document).ready(function() {
        const table = $('#clientsTable').DataTable({
            ...
            ordering: true,
            responsive: true,
            dom: 'tpr', // custom DOM placement, hides generic search/length
            columnDefs: [
                { orderable: false, targets: [0, 5] } // disable sorting for photo and action columns
            ]
        });
        ...
        // Link custom status filter
        $('#statusFilter').on('change', function() {
            const val = $(this).val();
            if (val === 'all') {
                table.column(3).search('').draw();
            } else {
                // Exact status column match (3 is the 'status' column index)
                const searchStr = val === 'active' ? 'Activo' : 'Inactivo';
                table.column(3).search(searchStr).draw();
            }
        });
    });
</script>
